...
public class ChangeCredentialsController {
    ...
    @PostMapping("/newCode")
    public ResponseEntity<OperationStatusResponse> generatePhoneResetCode(@RequestHeader Long userId){
        OperationStatusResponse response = changeCredentialsService.generatePhoneNumberResetCode(userId);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/code")
    public ResponseEntity<OperationStatusResponse> compareResetPasswordCode(@RequestBody CompareResetPasswordCodeRequest request, @RequestHeader Long userId){
